...
int main(int argc, char **argv)
{
  ...
  {
    ROS_INFO_STREAM("mmWaveQuickConfig: Configuring mmWave device using config file: " << argv[1]);
  }
  ...
  if (myParams.is_open())
  {
    ...
    {
      srv.request.comm.erase(std::remove(srv.request.comm.begin(), srv.request.comm.end(), '\r'),
      ...
      {
        ROS_INFO_STREAM("mmWaveQuickConfig: Sending command: " << srv.request.comm.c_str());
        if (client.call(srv))
        {
          ...
          else
          {
            ...
            ROS_ERROR_STREAM("mmWaveQuickConfig: Response: " << srv.response.resp.c_str());
            return 1;
          }
        }
        else
        {
          ...
          ROS_ERROR_STREAM("mmWaveQuickConfig: " << srv.request.comm.c_str());
          return 1;
        }
      }
    }
  ...
  {
    ROS_ERROR_STREAM("mmWaveQuickConfig: Failed to open File " << argv[1]);
    return 1;
  }
  ROS_INFO("mmWaveQuickConfig: mmWaveQuickConfig will now terminate.");
  ROS_INFO_STREAM("mmWaveQuickConfig: Done configuring mmWave device using config file: " << argv[1]);
